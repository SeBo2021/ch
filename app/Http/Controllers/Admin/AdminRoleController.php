...
//                'field' => 'query_like_name',
    ...
    public function permissions($permissions, $role = '')
    {
        ...
        //todo 优化成树状联动
        /*$permissions = tree($permissions,'id','parent_id','children',0);
        foreach ($permissions as $key1 => $item1) {
            $permissions[$key1]['own'] = $role ? $role->hasPermissionTo($item1['id']) ? 'checked' : false : false;
            $permissions[$key1]['title'] = $permissions[$key1]['cn_name'];
            if (isset($item1['children'])) {
                foreach ($item1['children'] as $key2 => $item2) {
                    $permissions[$key1]['children'][$key2]['own'] = $role ? $role->hasPermissionTo($item2['id']) ? 'checked' : false : false;
                    $permissions[$key1]['children'][$key2]['title'] = $permissions[$key1]['children'][$key2]['cn_name'];
                    if (isset($item2['children'])) {
                        foreach ($item2['children'] as $key3 => $item3) {
                            $permissions[$key1]['children'][$key2]['children'][$key3]['own'] = $role ? $role->hasPermissionTo($item3['id']) ? 'checked' : false : false;
                            $permissions[$key1]['children'][$key2]['children'][$key3]['title'] = $permissions[$key1]['children'][$key2]['children'][$key3]['cn_name'];
                        }
                    }
                }
            }
